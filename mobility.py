...
def fetch_mobility_pois(nyc_boundary):
    """
    Download mobility/transport POIs from OpenStreetMap for NYC.
    """
    print("\nFetching mobility/transport POIs from OpenStreetMap...")
    
    # Define mobility/transport tags
    tags = {
        'railway': [
            'station',
            'tram_stop'
    ...
    try:
        ...
        print(f"Downloaded {len(pois)} mobility/transport POIs from OSM.")
...
def spatial_join_and_aggregate(hex_gdf, pois_gdf):
    """
    Perform spatial join and count mobility/transport POIs per hexagon.
    """
    ...
    mobility_counts = joined.groupby('h3_id').size().reset_index(name='mobility_count')
    
    print(f"Found mobility/transport in {len(mobility_counts)} hexagons.")
    
    # Merge counts back to hex grid
    hex_gdf = hex_gdf.merge(mobility_counts, on='h3_id', how='left')
    
    # Fill NaN with 0
    hex_gdf['mobility_count'] = hex_gdf['mobility_count'].fillna(0)
...
def visualize_results(hex_gdf):
    ...
    """
    # Top 5 mobility hexagons
    top5 = hex_gdf.nlargest(5, 'mobility_count')[['h3_id', 'mobility_count']]
    print("\n=== TOP 5 MOBILITY/TRANSPORT HEXAGONS ===")
    for idx, row in top5.iterrows():
        print(f"  H3 ID: {row['h3_id']} - Mobility/Transport POIs: {int(row['mobility_count'])}")
    
    # Statistics
    print(f"\n=== MOBILITY/TRANSPORT STATISTICS ===")
    print(f"Total mobility/transport POIs: {int(hex_gdf['mobility_count'].sum())}")
    print(f"Hexagons with mobility: {int((hex_gdf['mobility_count'] > 0).sum())}")
    print(f"Max POIs in one hexagon: {int(hex_gdf['mobility_count'].max())}")
    if (hex_gdf['mobility_count'] > 0).sum() > 0:
        print(f"Average POIs per hexagon (with mobility): {hex_gdf[hex_gdf['mobility_count'] > 0]['mobility_count'].mean():.2f}")
    ...
    hex_gdf.plot(
        column='mobility_count', 
        ax=ax, 
        legend=True,
        cmap='Blues',  # Blue gradient for transport
        edgecolor='black',
        linewidth=0.1,
        legend_kwds={'label': 'Mobility/Transport POIs per Hexagon'}
    )
    
    plt.title("NYC Mobility & Transport Access by Hexagon (OSM Data, Resolution 8)")
...
def main():
    ...
    # Step 2: Fetch OSM mobility/transport POIs
    print("\n=== STEP 2: Fetch Mobility/Transport POIs from OpenStreetMap ===")
    pois_gdf = fetch_mobility_pois(nyc_boundary)
    ...
    pois_output = "data/processed/mobility_pois.gpkg"
    ...
    hex_output = "data/processed/mobility_hexagons.gpkg"
    print(f"\nSaving aggregated hexagon data to {hex_output}...")
    hex_gdf.to_file(hex_output, driver="GPKG")
    print(f"✓ Saved {len(hex_gdf)} hexagons with mobility_count column")
    
    # Step 5: Visualize
    print("\n=== STEP 5: Visualize Results ===")
    visualize_results(hex_gdf)
    
    print("\n✅ Mobility/Transport aggregation completed successfully!")
    print("Expected hotspots: Manhattan (Midtown), Downtown Brooklyn, Transit hubs")
    print("\n📁 Saved files:")
