...
def fetch_green_relax_pois(nyc_boundary):
    """
    Download green spaces & recreation POIs from OpenStreetMap for NYC.
    """
    print("\nFetching green spaces & recreation POIs from OpenStreetMap...")
    
    # Define green spaces & recreation tags
    tags = {
        'leisure': [
            # Urban green
            'park', 'garden', 'playground', 'dog_park',
            # Sports recreation
            'pitch', 'sports_centre', 'track', 'ice_rink',
    ...
    try:
        ...
        print(f"Downloaded {len(pois)} green space/recreation POIs from OSM.")
...
def spatial_join_and_aggregate(hex_gdf, pois_gdf):
    """
    Perform spatial join and count green space/recreation POIs per hexagon.
    """
    ...
    green_counts = joined.groupby('h3_id').size().reset_index(name='green_count')
    
    print(f"Found green spaces/recreation in {len(green_counts)} hexagons.")
    
    # Merge counts back to hex grid
    hex_gdf = hex_gdf.merge(green_counts, on='h3_id', how='left')
    
    # Fill NaN with 0
    hex_gdf['green_count'] = hex_gdf['green_count'].fillna(0)
...
def visualize_results(hex_gdf):
    ...
    """
    # Top 5 green space hexagons
    top5 = hex_gdf.nlargest(5, 'green_count')[['h3_id', 'green_count']]
    print("\n=== TOP 5 GREEN SPACE/RECREATION HEXAGONS ===")
    for idx, row in top5.iterrows():
        print(f"  H3 ID: {row['h3_id']} - Green/Recreation POIs: {int(row['green_count'])}")
    
    # Statistics
    print(f"\n=== GREEN SPACE/RECREATION STATISTICS ===")
    print(f"Total green space/recreation POIs: {int(hex_gdf['green_count'].sum())}")
    print(f"Hexagons with green spaces: {int((hex_gdf['green_count'] > 0).sum())}")
    print(f"Max POIs in one hexagon: {int(hex_gdf['green_count'].max())}")
    if (hex_gdf['green_count'] > 0).sum() > 0:
        print(f"Average POIs per hexagon (with green spaces): {hex_gdf[hex_gdf['green_count'] > 0]['green_count'].mean():.2f}")
    ...
    hex_gdf.plot(
        ...
        ax=ax, 
        legend=True,
        cmap='Greens',  # Green gradient for nature
        edgecolor='black',
        linewidth=0.1,
        legend_kwds={'label': 'Green Space/Recreation POIs per Hexagon'}
    )
    
    plt.title("NYC Green Spaces & Recreation by Hexagon (OSM Data, Resolution 8)")
...
def main():
    ...
    # Step 2: Fetch OSM green space/recreation POIs
    print("\n=== STEP 2: Fetch Green Space/Recreation POIs from OpenStreetMap ===")
    pois_gdf = fetch_green_relax_pois(nyc_boundary)
    ...
    # Clean up columns - keep only geometry and essential info
    # GeoPackage doesn't support column names with special characters like ':'
    print("\nCleaning column names for GeoPackage compatibility...")
    essential_cols = ['geometry']
    # Add any other simple columns that don't have special characters
    for col in pois_points.columns:
        if col != 'geometry' and ':' not in col and len(col) < 50:
            essential_cols.append(col)
    
    pois_clean = pois_points[essential_cols].copy()
    print(f"Kept {len(essential_cols)} columns (removed {len(pois_points.columns) - len(essential_cols)} problematic columns)")
    
    # Save raw POIs for future use
    os.makedirs("data/processed", exist_ok=True)
    pois_output = "data/processed/green_relax_pois.gpkg"
    print(f"\nSaving raw POI data to {pois_output}...")
    pois_clean.to_file(pois_output, driver="GPKG")
    print(f"✓ Saved {len(pois_clean)} POIs")
    ...
    hex_output = "data/processed/green_relax_hexagons.gpkg"
    print(f"\nSaving aggregated hexagon data to {hex_output}...")
    hex_gdf.to_file(hex_output, driver="GPKG")
    print(f"✓ Saved {len(hex_gdf)} hexagons with green_count column")
    
    # Step 5: Visualize
    print("\n=== STEP 5: Visualize Results ===")
    visualize_results(hex_gdf)
    
    print("\n✅ Green Space/Recreation aggregation completed successfully!")
    print("Expected hotspots: Central Park, Prospect Park, Staten Island")
    print("\n📁 Saved files:")
