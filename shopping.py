...
def fetch_shopping_pois(nyc_boundary):
    """
    Download shopping POIs from OpenStreetMap for NYC.
    """
    print("\nFetching shopping POIs from OpenStreetMap...")
    
    # Define shopping tags
    tags = {
        'shop': [
            'mall',
            'department_store',
    ...
    try:
        ...
        print(f"Downloaded {len(pois)} shopping POIs from OSM.")
...
def spatial_join_and_aggregate(hex_gdf, pois_gdf):
    """
    Perform spatial join and count shopping POIs per hexagon.
    """
    ...
    shopping_counts = joined.groupby('h3_id').size().reset_index(name='shopping_count')
    
    print(f"Found shopping in {len(shopping_counts)} hexagons.")
    
    # Merge counts back to hex grid
    hex_gdf = hex_gdf.merge(shopping_counts, on='h3_id', how='left')
    
    # Fill NaN with 0
    hex_gdf['shopping_count'] = hex_gdf['shopping_count'].fillna(0)
...
def visualize_results(hex_gdf):
    ...
    """
    # Top 5 shopping hexagons
    top5 = hex_gdf.nlargest(5, 'shopping_count')[['h3_id', 'shopping_count']]
    print("\n=== TOP 5 SHOPPING HEXAGONS (Shopping Districts!) ===")
    for idx, row in top5.iterrows():
        print(f"  H3 ID: {row['h3_id']} - Shopping POIs: {int(row['shopping_count'])}")
    
    # Statistics
    print(f"\n=== SHOPPING STATISTICS ===")
    print(f"Total shopping POIs: {int(hex_gdf['shopping_count'].sum())}")
    print(f"Hexagons with shopping: {int((hex_gdf['shopping_count'] > 0).sum())}")
    print(f"Max POIs in one hexagon: {int(hex_gdf['shopping_count'].max())}")
    if (hex_gdf['shopping_count'] > 0).sum() > 0:
        print(f"Average POIs per hexagon (with shopping): {hex_gdf[hex_gdf['shopping_count'] > 0]['shopping_count'].mean():.2f}")
    ...
    hex_gdf.plot(
        column='shopping_count', 
        ax=ax, 
        legend=True,
        cmap='Oranges',  # Orange gradient for shopping
        edgecolor='black',
        linewidth=0.1,
        legend_kwds={'label': 'Shopping POIs per Hexagon'}
    )
    
    plt.title("NYC Shopping Districts by Hexagon (OSM Data, Resolution 8)")
...
def main():
    ...
    # Step 2: Fetch OSM shopping POIs
    print("\n=== STEP 2: Fetch Shopping POIs from OpenStreetMap ===")
    pois_gdf = fetch_shopping_pois(nyc_boundary)
    ...
    pois_output = "data/processed/shopping_pois.gpkg"
    ...
    hex_output = "data/processed/shopping_hexagons.gpkg"
    print(f"\nSaving aggregated hexagon data to {hex_output}...")
    hex_gdf.to_file(hex_output, driver="GPKG")
    print(f"✓ Saved {len(hex_gdf)} hexagons with shopping_count column")
    
    # Step 5: Visualize
    print("\n=== STEP 5: Visualize Results ===")
    visualize_results(hex_gdf)
    
    print("\n✅ Shopping aggregation completed successfully!")
    print("Expected hotspots: Manhattan (Midtown, SoHo, Fifth Avenue)")
    print("\n📁 Saved files:")
