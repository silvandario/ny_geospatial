...
def fetch_restaurant_cafe_pois(nyc_boundary):
    """
    Download restaurant & café POIs from OpenStreetMap for NYC.
    """
    print("\nFetching restaurant & café POIs from OpenStreetMap...")
    
    # Define gastronomy tags
    tags = {
        'amenity': [
            'restaurant',   
            'cafe',        
    ...
    try:
        ...
        print(f"Downloaded {len(pois)} restaurant/café POIs from OSM.")
...
def spatial_join_and_aggregate(hex_gdf, pois_gdf):
    """
    Perform spatial join and count restaurant/café POIs per hexagon.
    """
    ...
    restaurant_counts = joined.groupby('h3_id').size().reset_index(name='restaurant_count')
    
    print(f"Found restaurants/cafés in {len(restaurant_counts)} hexagons.")
    
    # Merge counts back to hex grid
    hex_gdf = hex_gdf.merge(restaurant_counts, on='h3_id', how='left')
    
    # Fill NaN with 0
    hex_gdf['restaurant_count'] = hex_gdf['restaurant_count'].fillna(0)
...
def visualize_results(hex_gdf):
    ...
    """
    # Top 5 restaurant/café hexagons
    top5 = hex_gdf.nlargest(5, 'restaurant_count')[['h3_id', 'restaurant_count']]
    print("\n=== TOP 5 RESTAURANT/CAFÉ HEXAGONS (Foodie Zones!) ===")
    for idx, row in top5.iterrows():
        print(f"  H3 ID: {row['h3_id']} - Restaurant/Café POIs: {int(row['restaurant_count'])}")
    
    # Statistics
    print(f"\n=== RESTAURANT/CAFÉ STATISTICS ===")
    print(f"Total restaurant/café POIs: {int(hex_gdf['restaurant_count'].sum())}")
    print(f"Hexagons with restaurants/cafés: {int((hex_gdf['restaurant_count'] > 0).sum())}")
    print(f"Max POIs in one hexagon: {int(hex_gdf['restaurant_count'].max())}")
    if (hex_gdf['restaurant_count'] > 0).sum() > 0:
        print(f"Average POIs per hexagon (with restaurants): {hex_gdf[hex_gdf['restaurant_count'] > 0]['restaurant_count'].mean():.2f}")
    ...
    hex_gdf.plot(
        ...
        ax=ax, 
        legend=True,
        cmap='YlOrRd',  # Yellow-Orange-Red for food
        edgecolor='black',
        linewidth=0.1,
        legend_kwds={'label': 'Restaurant/Café POIs per Hexagon'}
    )
    
    plt.title("NYC Restaurant & Café Zones by Hexagon (OSM Data, Resolution 8)")
...
def main():
    ...
    # Step 2: Fetch OSM restaurant/café POIs
    print("\n=== STEP 2: Fetch Restaurant/Café POIs from OpenStreetMap ===")
    pois_gdf = fetch_restaurant_cafe_pois(nyc_boundary)
    ...
    pois_output = "data/processed/restaurant_cafe_pois.gpkg"
    ...
    hex_output = "data/processed/restaurant_cafe_hexagons.gpkg"
    print(f"\nSaving aggregated hexagon data to {hex_output}...")
    hex_gdf.to_file(hex_output, driver="GPKG")
    print(f"✓ Saved {len(hex_gdf)} hexagons with restaurant_count column")
    
    # Step 5: Visualize
    print("\n=== STEP 5: Visualize Results ===")
    visualize_results(hex_gdf)
    
    print("\n✅ Restaurant/Café aggregation completed successfully!")
    print("Expected hotspots: Manhattan (Midtown, Soho), Brooklyn (Williamsburg)")
    print("\n📁 Saved files:")
